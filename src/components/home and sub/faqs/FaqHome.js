...
import { faqHomeLinks } from "./linksData"


const FaqHome = () => {
    const renderLinks = () => (
        faqHomeLinks.map(item => <LinkHome key={item.id} title={item.title} link={item.link} />)
    )

    console.log(faqHomeLinks.length);
    return (
        <main className="w-full min-h-screen">
            <section className="max-w-4xl mx-auto my-4">
                ...
                <div className="flex flex-col mb-5">
                    ...
                    <p className="text-[#737373] text-[12px]">{faqHomeLinks.length} articles</p>
                </div>
