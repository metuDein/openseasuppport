...
import { userSafetyLinks } from "../faqs/linksData"

const UserSafetyFaqCenter = () => {
    const renderLinks = () => (
        userSafetyLinks.map(item => <LinkHome key={item.id} title={item.title} link={item.link} />)
    )
        ...
        <main className="w-full min-h-screen">
            <section className="max-w-4xl mx-auto my-4">
                ...
                <div className="flex flex-col mb-5">
                    ...
                    <p className="text-[#737373] text-[12px]">{userSafetyLinks.length} articles</p>
                </div>
...
export default UserSafetyFaqCenter